    ...
    if ( isset( $_POST['pt_submit'] ) ) {
       ...
       //--------------------------------------------
        ...
        if($stmt->execute()){
            ...
            $mailHandler = new \MailHandler2(get_option( 'admin_email' ),$email);        
            
            //-----------------sending mail-------------------------
            $subject = "Petition Title";
            $message = "Your Comfirmation link \r\n";
            $message.= "Dieser Link anclicken, um die Email zu den abgeordneten weiter zu schicken \r\n";
            $message.= plugins_url()."/confirmation.php?personalkey=$confirm_code";
            try{
                $mailHandler->sendMail($subject, $message,true);	
            }catch(Exception $e){
                //echo nl2br("Error: \n".$e->getMessage());
            }
        }else{
